...
export function toMarkdown(question: Question): string {
    let final: string = "# "+question.name+"\n"+question.body;
    if(question.type === "multiple_choice_question"){
        for(let i = 0;i<question.options.length;i++){
            final+= "\n"+ "- " + question.options[i];
        }
...
export function renameQuestion(question: Question, newName: string): Question {
    let new_question = {...question, name:newName}; 
    return new_question;
}
...
export function publishQuestion(question: Question): Question {
    let new_question = {...question, published: !question.published};
    return new_question;
}
...
export function duplicateQuestion(id: number, oldQuestion: Question): Question {
    let new_question = {...oldQuestion, id: id, name:"Copy of "+oldQuestion.name, published:false}
    return new_question;
}
...
export function mergeQuestion(
    ...
    let new_q = {id: id, name: name, body: contentQuestion.body, type:contentQuestion.type, options:contentQuestion.options,
